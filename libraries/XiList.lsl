    ...
    )
{
    ...
    return "Ξ" + XiInteger$ToHex( l, 1 ) + " " + out + " ";
}

list XiList$FromString( // converts a string generated by XiList$ToString(...) back into a list
    string in
    )
{
    if (llGetSubString(in, 0, 0) != "Ξ") return []; // not a XiList string
    ...
    if ( space > 7 ) return []; // impossible to have an XiInteger$ToHex result longer than 8 chars
    integer count;
