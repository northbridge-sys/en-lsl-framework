    ...
    )
{
    ...
    for ( i = 0; i < l; i++ )
    {
        ...
        out += enInteger_ToHex( llStringLength( elem ), 1 ) + " " + elem;
    }
    return "𒂗" + enInteger_ToHex( l, 1 ) + " " + out + " ";
}
    ...
    )
{
    ...
    if ( space > 7 ) return []; // impossible to have an enInteger_ToHex result longer than 8 chars
    integer count;
    ...
    { // for each element
        ...
        if ( space > 7 ) return []; // impossible to have an enInteger_ToHex result longer than 8 chars - signals non-enList string
