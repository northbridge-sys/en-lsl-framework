...
#define enList$FindStrideByElem( list_haystack, stride_length, index_in_stride, string_needle ) \
...
string enList$Elem(list var)
{
    if (var == []) return "[]";
    return "[\"" + llDumpList2String(var, "\", \"") + "\"]";
}

list enList$Empty( // if a list only has one element that is a blank string, convert it to an empty list
    list in
    )
{
...
)
{
    ...
    for (i = 0; i < l; i++)
    {
        out += ["\"" + enString$Escape(ENSTRING$ESCAPE_FILTER_JSON, llList2String(in, i)) + "\""];
    }
    return out;
}
...
string enList$ToString( // converts a list into a string without worrying about separators or JSON
    list in
    )
{
    string out;
    ...
    for ( i = 0; i < l; i++ )
    {
        ...
        out += enInteger$ToHex( llStringLength( elem ), 1 ) + " " + elem;
    }
    return "Ξ" + enInteger$ToHex( l, 1 ) + " " + out + " ";
}

list enList$FromString( // converts a string generated by enList$ToString(...) back into a list
    string in
    )
{
    if (llGetSubString(in, 0, 0) != "Ξ") return []; // not a enList string
    ...
    if ( space > 7 ) return []; // impossible to have an enInteger$ToHex result longer than 8 chars
    integer count;
    ...
    string elem;
    list out;
    while ( length > 1 && space && elem_length)
    { // for each element
        space = llSubStringIndex( in, " " ); // find header delineator - hex before this is length of element body
        if ( space > 7 ) return []; // impossible to have an enInteger$ToHex result longer than 8 chars - signals non-enList string
...
integer enList$FindPartial( // llListFindList but needle can be only part of the element instead of the entire element
    list x,
    string s
    )
{
...
list enList$DeleteStrideByMatch(
    list haystack,
    integer stride,
    integer index,
    list needle
